...
function loadDataFromFirebase() {
    ...
    get(child(dbRef, '/teaStatus')).then((snapshot) => {
        if (snapshot.exists()) {
...
function submitFeedback(event) {
    ...
    database.ref('/feedbacks').push(feedback).then(() => {
...
function updateTeaPackage() {
    ...
    database.ref('/teaStatus/packageCount').set(packageCount).then(() => {
        document.getElementById('tea-package-status').innerText = 
...
function updateTeaTimes() {
    ...
    database.ref('/teaStatus/times').set(times).then(() => {
